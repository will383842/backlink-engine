// ---------------------------------------------------------------------------
// v2 message templates — unified blog + default.
//
// William's latest rule (2026-04-20):
//   - Same body for blog templates AND default templates
//   - Two links: "Découvrez la plateforme: sos-expat.com" + affiliate link
//   - FR keeps phone; 8 other languages don't
//   - No variable placeholders
//
// Upserts BOTH:
//   - sourceContactType = 'blog'  (blog-specific, existing 9 rows)
//   - sourceContactType = null, category = null  (default fallback, 9 rows)
// ---------------------------------------------------------------------------
...
Inscrivez-vous comme affilié : sos-expat.com/devenir-blogger
...
Sign up as an affiliate: sos-expat.com/devenir-blogger
...
Regístrate como afiliado: sos-expat.com/devenir-blogger
...
Melden Sie sich als Affiliate an: sos-expat.com/devenir-blogger
...
Inscreva-se como afiliado: sos-expat.com/devenir-blogger
...
Зарегистрируйтесь как партнёр: sos-expat.com/devenir-blogger
...
سجّل كشريك تابع: sos-expat.com/devenir-blogger
...
了解平台：sos-expat.com
注册成为推广合作伙伴：sos-expat.com/devenir-blogger
...
एफ़िलिएट के रूप में पंजीकरण करें: sos-expat.com/devenir-blogger
...
async function upsertBlog(tpl: LangTemplate, frId: number | null) {
...
async function upsertDefault(tpl: LangTemplate, frId: number | null) {
  const existing = await prisma.messageTemplate.findFirst({
    where: { language: tpl.language, sourceContactType: null, category: null },
  });
  if (existing) {
    return prisma.messageTemplate.update({
      where: { id: existing.id },
...
async function main() {
  ...
  console.log("=== Step 1: update blog templates (9 rows) ===");
  const frBlog = await upsertBlog(fr, null);
  console.log(`  ✓ fr  blog    → id=${frBlog.id}`);
  for (const tpl of TEMPLATES.filter((t) => t.language !== "fr")) {
    const saved = await upsertBlog(tpl, frBlog.id);
    console.log(`  ✓ ${tpl.language.padEnd(3)} blog    → id=${saved.id}`);
  }

  console.log("\n=== Step 2: update default templates (9 rows) ===");
  const frDefault = await upsertDefault(fr, null);
  console.log(`  ✓ fr  default → id=${frDefault.id}`);
  for (const tpl of TEMPLATES.filter((t) => t.language !== "fr")) {
    const saved = await upsertDefault(tpl, frDefault.id);
    console.log(`  ✓ ${tpl.language.padEnd(3)} default → id=${saved.id}`);
  }

  console.log("\n=== Verification ===");
  const blogRows = await prisma.messageTemplate.findMany({
    where: { sourceContactType: "blog" },
    orderBy: { language: "asc" },
  });
  const defaultRows = await prisma.messageTemplate.findMany({
    where: { sourceContactType: null, category: null },
    orderBy: { language: "asc" },
  });

  console.log(`\nblog templates (${blogRows.length}):`);
  for (const t of blogRows) {
    const hasPhone = t.body.includes("+33") ? "AVEC phone" : "sans phone";
    const hasPlatformLink = t.body.includes("sos-expat.com\n") || t.body.includes("sos-expat.com\r\n") ? "✓ platform" : "";
    const hasAffLink = t.body.includes("devenir-blogger") ? "✓ affiliate" : "";
    console.log(`  ${t.language.padEnd(3)} ${t.body.length} chars ${hasPhone} ${hasPlatformLink} ${hasAffLink}`);
  }
  console.log(`\ndefault templates (${defaultRows.length}):`);
  for (const t of defaultRows) {
    const hasPhone = t.body.includes("+33") ? "AVEC phone" : "sans phone";
    const hasPlatformLink = t.body.includes("sos-expat.com\n") || t.body.includes("sos-expat.com\r\n") ? "✓ platform" : "";
    const hasAffLink = t.body.includes("devenir-blogger") ? "✓ affiliate" : "";
    console.log(`  ${t.language.padEnd(3)} ${t.body.length} chars ${hasPhone} ${hasPlatformLink} ${hasAffLink}`);
